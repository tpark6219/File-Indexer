...
/* Must create a node class so that we can construct a linked list Data structure...
...
struct SortedList
{
       ...
       DestructFuncT destructionator;
                                //Size!! So we know whe nto delete an iterator.
                      };
...
{
    //We need the a sortedList to iterate through -> thus we need access to the head via SortedListPtr?
    //Need to keep track of what node we are on.
    struct node* curr;
